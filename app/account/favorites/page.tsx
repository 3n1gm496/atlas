...
export default async function AccountFavoritesPage() {
  ...
  const favorites = user ? await prisma.favorite.findMany({ where: { userId: user.id }, include: { entry: true }, take: 50 }).catch(() => []) : [];

  return (
    <section className="space-y-4">
      <h1 className="atlas-title">Preferiti</h1>
      <div className="space-y-2">
        {favorites.map((f) => <Link key={f.id} href={`/entry/${f.entry.slug}`} className="atlas-card block">{f.entry.title}</Link>)}
